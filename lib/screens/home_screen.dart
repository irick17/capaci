...
// コーチマーク用に追加 & [TODO 3] appSettingsProvider
...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  void _showCoachMark(BuildContext context) {
    ...
    try { // コーチマーク表示も try-catch
      ...
      logger.d("Onboarding complete: $isOnboardingComplete, Coach mark shown: $isCoachMarkShown"); // Log values
           ...
           if (mounted) { // Check mounted again inside microtask
             try {
               ...
               logger.d("Coach mark started and marked as shown.");
                ...
                logger.e("Error starting showcase or marking as shown", error: e, stackTrace: stackTrace);
             }
           } else {
              logger.w("Showcase start skipped: widget unmounted before microtask execution.");
           }
        });
  ...
  void _scheduleNotifications() {
    logger.d("Attempting to schedule notifications...");
    try {
      final notificationService = ref.read(notificationServiceProvider);
      // *** [TODO 3] appSettingsProvider から設定を読み込む ***
      final settings = ref.read(appSettingsProvider);

      // (設定変更に備え) 既存の通知を一度キャンセルしてから再スケジュールする
      // (注: IDが固定 (100, 101) なので、時刻変更時は上書きされるが、
      //  通知OFF->ON時に備えてキャンセルするのが確実)
      logger.d("Cancelling existing reminders before rescheduling...");
      // *** エラー修正: kTest... -> NotificationService.morningReminderId ***
      notificationService.cancelNotification(NotificationService.morningReminderId);
      notificationService.cancelNotification(NotificationService.eveningReminderId);

      // [TODO 3] settings を渡してスケジュール
      // *** エラー修正: settings.reminderTimeMorning -> settings.morningTime ***
      // *** エラー修正: settings.reminderTimeEvening -> settings.eveningTime ***
      logger.d("Scheduling reminders with settings: Enabled=${settings.notificationsEnabled}, Morning=${settings.morningTime}, Evening=${settings.eveningTime}");
      notificationService.scheduleTestReminders(settings);

      logger.d("Notification scheduling complete.");
    } catch (e, stackTrace) {
      logger.e("Error scheduling notifications", error: e, stackTrace: stackTrace);
    }
  }
  // *** [TODO 2] ここまで ***
  ...
  Widget build(BuildContext context) {
    ...
    final currentCycleIndex = ref.watch(currentCycleIndexProvider); // Watch the index
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          // *** [TODO 3] 設定画面への導線 ***
          IconButton(
            icon: Icon(Icons.settings_outlined, // 設定アイコン
                 color: isGoldenTime ? colorScheme.onPrimaryContainer : colorScheme.onSurfaceVariant),
            tooltip: '設定',
            onPressed: () {
               logger.i("Settings button pressed. Navigating to SettingsScreen...");
               Navigator.push(
                 context,
             ...
             child: Visibility(
               ...
               child: Card(
                 ...
                 shape: RoundedRectangleBorder(
                     borderRadius: BorderRadius.circular(16.0)), // M3 card radius
                 // (修正) const を追加
                 ...
                 child: Padding( // Add padding inside the card
                   ...
                   child: Row( // Use Row for icon, text, and optional graphic
                     children: [
                       ...
                       Expanded( // Allow text to take available space
                         child: Column(
                           ...
                           children: [
                             ...
                             Text(AppStrings.goldenTimeBody,
                       ...
                       Row(
                         ...
                         children: [
                           ...
                           Icon(Icons.adjust, color: colorScheme.onPrimaryContainer.withAlpha(200), size: 20), // Placeholder for Sperm
                           Padding(
                             padding: const EdgeInsets.symmetric(horizontal: 4.0),
                             child: Icon(Icons.arrow_forward_ios_rounded, color: colorScheme.onPrimaryContainer.withAlpha(180), size: 16),
                           ),
                           Icon(Icons.hourglass_bottom_rounded, color: colorScheme.onPrimaryContainer.withAlpha(200), size: 20), // Placeholder for Capacitation Time
                           Padding(
                             padding: const EdgeInsets.symmetric(horizontal: 4.0),
                             child: Icon(Icons.arrow_forward_ios_rounded, color: colorScheme.onPrimaryContainer.withAlpha(180), size: 16),
                           ),
                            Icon(Icons.trip_origin, color: colorScheme.onPrimaryContainer.withAlpha(200), size: 20), // Placeholder for Egg
                         ],
                       )
                     ],
                   ),
          ...
          Expanded(
            child: PageView.builder(
              ...
              itemBuilder: (context, index) {
                 logger.d("PageView itemBuilder: Building page for index $index"); // Log item build
                // V1 (フロー 5) ページ切り替え
                // (修正) index から cycleId を取得して渡す
                // (修正) cycleList を使用
                 final String? expectedId = (index >= 0 && index < cycleList.length) ? cycleList[index].id : null;
                 logger.d("Expecting cycle ID for index $index: $expectedId");
                return _buildCyclePage(
                    context, index, currentCycleIndex == index, expectedId, cycleList); // Pass expected ID and cycleList
              },
            ),
          ),
        ],
  ...
  /// (修正) expectedId を受け取る
  /// (修正) cycleList を受け取る
  Widget _buildCyclePage(BuildContext context, int pageIndex, bool isActive, String? expectedId, List<models.CycleData> cycleList) {
     logger.d("_buildCyclePage for index $pageIndex, isActive: $isActive. Watching currentCycleDataProvider.");
    // V1 (P1/P0)
    ...
    // Define padding based on active status for "peek" effect
     final horizontalPadding = isActive ? 8.0 : 16.0; // More padding when inactive
    ...
    return Padding(
      ...
      child: IgnorePointer(
         ignoring: !isActive,
         // (修正) const を追加
         child: AnimatedOpacity( // Use AnimatedOpacity for smooth transition
              ...
              child: currentCycleDataAsync.when(
                data: (cycleData) {
                  ...
                  if (cycleData == null || cycleData.id != expectedId) {
                     logger.d("_buildCyclePage (data): pageIndex=$pageIndex, expectedId=$expectedId, receivedId=${cycleData?.id}. Data is null or ID mismatch, showing empty or loading.");
                     // Show a simple loading or empty state during mismatch
                      // (修正) cycleList を使用
                      if (cycleList.isEmpty) { // Show initial empty state if no cycles exist at all
                          // (修正) const を追加
                          return const _EmptyState(
                             title: AppStrings.emptyStateTitleB, // Or specific title for no data
                   ...
                   logger.d("_buildCyclePage (data): pageIndex=$pageIndex, ID matched (${cycleData.id}). Records: ${cycleData.records?.length ?? 0}");

                  // Handle case where cycle exists but has no records yet (common after onboarding)
                  if (cycleData.records == null || cycleData.records!.isEmpty) {
                     logger.d("Showing empty state for cycle ${cycleData.id} (no records).");
                     // Show empty state specific to having a cycle but no records
                     // (修正) const を追加
                     return const _EmptyState(
                       title: AppStrings.emptyStateTitleA, // "あなたの「安心グラフ」です"
                       body: AppStrings.emptyStateBodyA,   // "まずは..."
                     );
                  }
                ...
                error: (e, stackTrace) {
                  logger.e("_buildCyclePage (error): pageIndex=$pageIndex", error: e, stackTrace: stackTrace);
  ...
  Widget _buildChart(BuildContext context, models.CycleData cycleData) {
    ...
    final records = (cycleData.records?.toList() ?? [])
     ...
     logger.d("Building chart for cycle ${cycleData.id} with ${records.length} sorted records.");

    // グラフのX軸（日付）の範囲を計算
    final startDate = cycleData.startDate;
    // Ensure averageCycleLength is positive
    final cycleLengthForCalc = cycleData.averageCycleLength > 0 ? cycleData.averageCycleLength : 28; // Default to 28 if invalid
    ...
    final predictedBbt =
     ...
     logger.d("Generated ${predictedLh.length} LH predictions, ${predictedBbt.length} BBT predictions.");
    ...
    final List<ChartData> predictedSpermZones = spermZones['predicted']!;
     logger.d("Actual sperm zones: ${actualSpermZones.length} points, Predicted: ${predictedSpermZones.length} points.");
    ...
    // (修正) const を追加
    return Padding(
      padding: const EdgeInsets.only(top: 24, right: 16, bottom: 12, left: 8), // Added left padding
      child: SfCartesianChart(
        ...
        primaryYAxis: NumericAxis(
          title: AxisTitle(
              text: 'LH',
              textStyle: theme.textTheme.labelSmall?.copyWith(color: colorScheme.primary)),
          minimum: 0,
          ...
          // (修正) const を追加
          majorGridLines: const MajorGridLines(width: 0.5), // Subtle grid lines matching X-axis
          ...
          axisLabelFormatter: (AxisLabelRenderDetails details) {
             ...
             if (value >= 0 && value < models.TestResult.values.length) {
                switch(models.TestResult.values[value]) {
                  case models.TestResult.none: label = ''; break; // Or '-'
                  case models.TestResult.negative: label = '陰性'; break;
                  case models.TestResult.positive: label = '陽性'; break;
                  case models.TestResult.strongPositive: label = '強陽'; break;
                  // (修正) default case を削除
                }
             } else {
                 label = '';
          ...
          labelStyle: theme.textTheme.labelSmall?.copyWith(color: colorScheme.primary),
        ),
        // BBT Y-Axis (Right)
        axes: <ChartAxis>[
          NumericAxis(
            ...
            title: AxisTitle(
                text: 'BBT (℃)',
                textStyle: theme.textTheme.labelSmall?.copyWith(color: colorScheme.secondary)),
            ...
            majorTickLines: const MajorTickLines(size: 0),
            // (修正) const を追加
             majorGridLines: const MajorGridLines(width: 0), // Hide grid lines for this axis
  ...
  ColumnSeries<models.CycleRecord, DateTime> _buildLhSeries(
    ...
    return ColumnSeries<models.CycleRecord, DateTime>(
      ...
      yValueMapper: (models.CycleRecord record, _) => record.testResult.index.toDouble(),
      ...
      pointColorMapper: (models.CycleRecord record, _) {
          ...
          // (修正) withAlpha を使う
          return colorScheme.primary.withAlpha(isPrediction ? (255 * 0.3).round() : 255); // Less opaque for prediction
      },
      // (修正) const を追加
       borderRadius: const BorderRadius.all(Radius.circular(4)), // Rounded corners for bars
    );
  }
      ...
      // Use pointColorMapper for marker color/opacity
       pointColorMapper: (models.CycleRecord record, _) {
          ...
          // V1.2 (2.1) 予測マーカーは半透明 (Opacity 0.5), 確定マーカーは不透明 (Opacity 1.0)
           // (修正) withAlpha を使う
          return colorScheme.secondary.withAlpha(isPrediction ? (255 * 0.5).round() : 255);
      },
      yAxisName: 'bbtAxis',
  ...
  ScatterSeries<models.CycleRecord, DateTime> _buildTimingMarkers(
    ...
    return ScatterSeries<models.CycleRecord, DateTime>(
      ...
      yValueMapper: (models.CycleRecord record, _) => -0.5, // Position below the LH bars
      ...
      markerSettings: const MarkerSettings(
        ...
        // iconType: IconType.favorite,
         shape: DataMarkerType.circle, // Changed to Circle for potentially better rendering
  ...
  RangeAreaSeries<ChartData, DateTime> _buildOvulationZoneSeries(
    ...
    final Color zoneColor = isConfirmed
      ...
      // (修正) withAlpha を使う
      ? colorScheme.error.withAlpha((255 * 0.3).round()) // Adjust opacity as needed
    ...
    return RangeAreaSeries<ChartData, DateTime>(
      ...
      highValueMapper: (ChartData data, _) => 5, // Extend slightly above 4 (LH max)
      name: isConfirmed ? AppStrings.helpLegendOvulationConfirmed : AppStrings.helpLegendOvulation, // Dynamic name
      color: zoneColor,
  ...
  void _showRecordModal(BuildContext context, WidgetRef ref) {
     ...
     logger.d("Current index: ${ref.read(currentCycleIndexProvider)}, Total cycles: ${ref.read(cycleDataProvider).length}"); // Log index and count
     ...
     switch (cycleDataAsync) {
        case AsyncData(:final value):
           if (value != null) {
              logger.d("Found cycle data for index ${ref.read(currentCycleIndexProvider)}: ID=${value.id}");
              ...
              logger.d("Showing RecordModal for cycle ID: $cycleId");
               ...
               // Proceed to show modal
                showModalBottomSheet(
                  ...
                  useSafeArea: true,
                  builder: (modalContext) { // Use a different context name
                    ...
                    return RecordModal(
                      ...
                      onSubmit: (record) {
                         logger.d("RecordModal onSubmit: date=${record.date}, bbt=${record.bbt}, test=${record.testResult}, timing=${record.isTiming}, period=${record.isPeriod}"); // [TODO 4] Log period
                          ...
                          Future(() async {
                             try {
                                ...
                                logger.d("addOrUpdateRecord successful for cycle $cycleId.");
                                ...
                                if (record.testResult == models.TestResult.positive ||
                                  ...
                                  ScaffoldMessenger.of(modalContext).showSnackBar(
                                    ...
                                    const SnackBar(
                                      ...
                                      // (修正) const を追加
                                      duration: Duration(seconds: 3), // Slightly longer duration
                                    ),
                                  );
                                } else {
                                ...
                                logger.e("Error calling addOrUpdateRecord", error: e, stackTrace: stackTrace);
                                // Show error SnackBar
               ...
               logger.e("!!! Cycle data is null within AsyncData. Provider State: ${ref.read(cycleDataProvider)}");
               // (修正) const を追加
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('現在の周期データが見つかりません。(Error Code: AD)')),
                );
            }
           break; // Added break
       case AsyncLoading():
          ...
          // (修正) const を追加
           ScaffoldMessenger.of(context).showSnackBar(
             // (修正) const を追加
             const SnackBar(content: Text('データを読み込み中です...'), duration: Duration(seconds: 1)),
           );
          break; // Added break
       case AsyncError(:final error, :final stackTrace):
          logger.e("Cannot show record modal due to error in cycle data provider.", error: error, stackTrace: stackTrace);
           ScaffoldMessenger.of(context).showSnackBar(
     ...
     switch (cycleDataAsync) {
        case AsyncData(:final value):
           if (value != null) {
                         ...
                         // Use async/await
                          Future(() async {
                              try {
                                ...
                                logger.d("addTimingRecord successful for cycle $cycleId.");
                                ...
                                ScaffoldMessenger.of(modalContext).showSnackBar(
                                 ...
                                 logger.e("Error calling addTimingRecord", error: e, stackTrace: stackTrace);
                                 if (modalContext.mounted) {
                                    ScaffoldMessenger.of(modalContext).showSnackBar(
                                       SnackBar(content: Text('タイミング記録の保存に失敗しました: $e')),
                                    );
                                 }
                              }
                          });
              ...
              logger.e("!!! Cycle data is null within AsyncData for timing modal.");
              // (修正) const を追加
               ScaffoldMessenger.of(context).showSnackBar(
                 const SnackBar(content: Text('現在の周期データが見つかりません。(Error Code: T-AD)')),
               );
           }
          break; // Added break
       case AsyncLoading():
          logger.w("Attempted to show timing modal while cycle data is loading.");
          // (修正) const を追加
           ScaffoldMessenger.of(context).showSnackBar(
             // (修正) const を追加
             const SnackBar(content: Text('データを読み込み中です...'), duration: Duration(seconds: 1)),
          ...
          break; // Added break
       case AsyncError(:final error, :final stackTrace):
           logger.e("Cannot show timing modal due to error in cycle data provider.", error: error, stackTrace: stackTrace);
           ScaffoldMessenger.of(context).showSnackBar(
             SnackBar(content: Text('周期データの読み込みエラー: $error')),
           );
          break; // Added break
  ...
  void _buildHelpModal(
    ...
    showModalBottomSheet(
      ...
      // (修正) const を追加
       shape: const RoundedRectangleBorder( // Consistent M3 shape
      ...
      builder: (context) {
        ...
        return SingleChildScrollView(
          // (修正) const を追加
           child: Padding(
            padding: const EdgeInsets.fromLTRB(24.0, 8.0, 24.0, 24.0), // Adjust top padding
            child: Column(
              mainAxisSize: MainAxisSize.min,
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                _buildLegendItem(
                  colorScheme.errorContainer.withAlpha(77), // Prediction color Opacity(0.3)
                 ...
                 // BBT確定時の凡例を追加
                _buildLegendItem(
                  colorScheme.error.withAlpha(77), // Confirmed color Opacity(0.3)
                ...
                _buildLegendItem(
                  colorScheme.primary.withAlpha(38), // Actual Sperm Bar Opacity(0.15)
                 ...
                 // (TODO 4: 予測バーの凡例を追加)
                _buildLegendItem(
                  colorScheme.tertiaryContainer.withAlpha(51), // Predicted Sperm Bar Opacity(0.2)
  ...
  Widget _buildLegendItem(
      ...
      {bool isBar = false, bool isLine = false, bool isArea = false, bool isMarker = false, bool isDashed = false}) {
    ...
    if (isArea) {
      leadingWidget = Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(4),
           border: isDashed ? Border.all(color: color.withAlpha(150), width: 1, style: BorderStyle.solid) : null, // Add border for dashed look if needed
        ),
       ...
       leadingWidget = SizedBox(
         ...
         child: CustomPaint(
            ...
            child: isDashed ? null : Center( // Show marker only for solid line if needed
      ...
      leadingWidget = Icon(Icons.favorite, color: color, size: iconSize); // Use favorite icon for timing
    }
     else {
      // Default square
      leadingWidget = Container(
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.insights_rounded,
                size: 48, color: theme.colorScheme.primary.withAlpha(150)), // Softer color
            // (修正) const を追加
            const SizedBox(height: 16),
            Text(
              title,
...
class LinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (size.width <= 0 || size.height <= 0) {
      logger.w("LinePainter: Skipping paint because size is zero or negative ($size)");
      return;
    }
    // *** 修正ここまで ***
