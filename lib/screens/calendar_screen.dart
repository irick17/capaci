...
// *** 修正: 'intl.dart' はこのファイルでは使用されていないため削除 ***
// import 'package:intl/intl.dart';
import 'package:table_calendar/table_calendar.dart';

// *** TODO 1: 予測ロジックをインポート ***
// *** 修正: isSameDay が競合するため hide を追加 ***
...
// *** 修正: 競合する 'prediction_logic.dart' の import (line 12) は不要なため削除 ***
// import '../utils/prediction_logic.dart' hide isSameDay, ChartData;
...
class CalendarEvent {
  ...
  // *** TODO 1: 予測排卵ゾーン用のフラグ ***
  final bool isOvulationZone; // 予測排卵ゾーンのマーカーかどうか
  ...
  CalendarEvent(
    ...
    this.icon,
    this.subtitle,
    this.isOvulationZone = false, // デフォルトは false
  });
...
class _CalendarScreenState extends ConsumerState<CalendarScreen> {
  ...
  final DateTime _firstDay =
      DateTime.now().subtract(const Duration(days: 365 * 2)); // 過去2年
  final DateTime _lastDay =
      DateTime.now().add(const Duration(days: 365)); // 未来1年
  ...
  void initState() {
    ...
    _predictionLogic = PredictionLogic(); // インスタンスを初期化
  ...
  void _loadEventsFromProvider() {
    ...
    if (!mounted) {
      logger.w(
          "CalendarScreen: _loadEventsFromProvider called but widget not mounted.");
      return;
    }
    ...
    for (final cycle in allCycles) {
      ...
      for (int i = 0; i < 5; i++) {
        ...
        final event = CalendarEvent(
            dayKey, "生理", colorScheme.errorContainer.withAlpha(200),
      ...
      final Map<String, dynamic> ovulationZoneInfo =
          _predictionLogic.getOvulationZones(cycle, records); // ソート済み records を使用
      // *** 修正: PredictionChartData -> ChartData (型名を修正) ***
      ...
      if (ovulationZones.isNotEmpty) {
        // ゾーンの開始日と終了日を取得 (ChartData は 0:start, 2:end と想定)
        final zoneStart = _normalizeDate(ovulationZones[0].x);
        final zoneEnd = _normalizeDate(ovulationZones[2].x);
        final zoneColor = isOvulationConfirmed
            ? colorScheme.error // 確定 (濃い色)
            : colorScheme.errorContainer; // 予測 (薄い色)

        // ゾーンの期間中、毎日ドットマーカーを追加
        for (var day = zoneStart;
            day.isBefore(zoneEnd.add(const Duration(days: 1)));
            day = day.add(const Duration(days: 1))) {
          final dayKey = _normalizeDate(day);
          // putIfAbsent を先に呼び出してリストを確実に初期化
          ...
          final event = CalendarEvent(
            ...
            zoneColor,
            isOvulationZone: true, // ゾーンフラグを立てる
          );
          // 既存のイベントと重複しないように追加 (例: 生理と重なっても両方表示)
          if (!newEventsMap[dayKey]!.any((e) => e.isOvulationZone)) {
      ...
      if (records.isNotEmpty) {
        for (final record in records) {
          ...
          // LH陽性/強陽性
          if (record.testResult == TestResult.positive ||
              record.testResult == TestResult.strongPositive) {
            final event = CalendarEvent(
                dayKey, "LH ${record.testResult.name}", colorScheme.primary,
          ...
          if (record.isTiming) {
            final event = CalendarEvent(
                dayKey, AppStrings.helpLegendTiming, colorScheme.tertiary,
                icon: Icons.favorite);
            if (!newEventsMap[dayKey]!
                .any((e) => e.title == AppStrings.helpLegendTiming)) {
              newEventsMap[dayKey]!.add(event);
            }
          }
          ...
          if (record.bbt != null) {
            final event = CalendarEvent(
                dayKey, "BBT", colorScheme.secondary,
                icon: Icons.thermostat_outlined,
                subtitle: "${record.bbt?.toStringAsFixed(2)} ℃");
  ...
  void _onDaySelected(DateTime selectedDay, DateTime focusedDay) {
    ...
    // *** 修正: ここで table_calendar の isSameDay を使用 (競合解消) *** (TODO は完了とみなす)
      ...
      body: Column(
        children: [
          TableCalendar<CalendarEvent>(
            ...
            // *** 修正: ここで table_calendar の isSameDay を使用 (競合解消) ***
            ...
            calendarStyle: CalendarStyle(
              ...
              // --- 週末のスタイル ---
              weekendTextStyle:
                  TextStyle(color: colorScheme.primary.withAlpha(200)),
                  ...
                  return Positioned(
                    ...
                    child: Row(
                      ...
                      children: events.take(4).map((event) {
                        ...
                        if (event.icon != null && !event.isOvulationZone) {
                          // アイコンマーカー
                          return Padding(
                          ...
                          // 通常の点マーカー (または isOvulationZone のドット)
                          return Container(
                            width: 6,
                            height: 6,
                            margin: const EdgeInsets.symmetric(horizontal: 1.0),
                            decoration: BoxDecoration(
                              // (isOvulationZone の場合は色をそのまま使用)
                              color: event.color,
                              shape: BoxShape.circle,
                            ),
                          );
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Card(
                        ...
                        child: ListTile(
                          ...
                          leading: (event.icon != null && !event.isOvulationZone)
                              ? Icon(event.icon,
                                  color: event.color, size: 28)
                              : (event.isOvulationZone // ゾーンイベントか
                                  ? Padding(
                                      padding: const EdgeInsets.all(
                                          10.0), // アイコンのサイズ感に合わせる
                                      child: Container(
                                        width: 8, height: 8, // 少し小さめ
                                        decoration: BoxDecoration(
                                          color: event.color,
                                          shape: BoxShape.circle,
                                        ),
